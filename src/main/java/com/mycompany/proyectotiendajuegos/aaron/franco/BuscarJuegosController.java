...
public class BuscarJuegosController implements Initializable {
    ...
    @Override
    public void initialize(URL url, ResourceBundle rb) {cbTipo.setItems(FXCollections.observableArrayList("Nombre", "Género", "Director", "Estudio")); 
    ...
    private void configurarColumnas() {
        colTitulo.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getTitulo()));                   //MAdre mia quien me manda a mi a nombrar estas variables con una letra para ahorrarme escribir
        colGenero.setCellValueFactory(c ->new SimpleStringProperty(c.getValue().getGenero()));
        colPlataforma.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getPlataforma()));
        colDirector.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getDirector()));
        colMedia.setCellValueFactory(c -> {double m = gd.getPuntuacionMediaJuego(c.getValue().getIdJuego()); //m = puntuacionMedia
        return new SimpleStringProperty(m > 0 ? String.format("%.1f", m) : "–");});                          //s = stock
        colPrecio.setCellValueFactory(c -> new SimpleStringProperty(String.format("%.2f€", c.getValue().getPrecio())));
        colStock.setCellValueFactory(c -> {int s = c.getValue().getStock(); return new SimpleStringProperty(s > 0 ? String.valueOf(s) : "Agotado");});
    ...
    private void mostrarDetalle(Juego j) {
        ...
        boolean loTiene = gd.usuarioPoseeJuego(gd.getUsuarioActual().getIdUsuario(), j.getIdJuego());

        lblEnBiblioteca.setVisible(loTiene);
        lblEnBiblioteca.setManaged(loTiene);
        btnComprar.setVisible(!loTiene);  //Para que no pueda comprar juegos que ya tiene, que antes podía y se liaba un poquillo
